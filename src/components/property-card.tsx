    ...
    <article className="property-card">
      <div className="property-image">
        ...
        <div className="property-image-top">
          <span className={property.verified ? "status-badge verified" : "status-badge"}>{property.verified && <BadgeCheck size={13} />}{property.verified ? "Verified" : "Editorial profile"}</span>
          <CompareToggle id={property.id} compact />
        </div>
      </div>
      <div className="property-content">
        <div className="property-meta"><span><MapPin size={14} /> {property.locality}, {city?.name}</span>{property.reviewCount > 0 && <span><Star size={13} fill="currentColor" /> {property.rating} ({property.reviewCount})</span>}</div>
