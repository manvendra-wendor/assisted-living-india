...
export default async function ArticlePage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  return <>
    <JsonLd data={[{ "@context": "https://schema.org", "@type": "Article", headline: article.title, description: article.excerpt, image: article.image, datePublished: article.publishedAt, dateModified: article.updatedAt ?? article.publishedAt, author: { "@type": "Organization", name: "Assisted Living India Editorial Team", url: `${siteConfig.url}/editorial-policy` }, publisher: { "@type": "Organization", name: "Assisted Living India", url: siteConfig.url }, mainEntityOfPage: `${siteConfig.url}/blog/${article.slug}` }, { "@context": "https://schema.org", "@type": "BreadcrumbList", itemListElement: [{ "@type": "ListItem", position: 1, name: "Home", item: siteConfig.url }, { "@type": "ListItem", position: 2, name: "Guides", item: `${siteConfig.url}/blog` }, { "@type": "ListItem", position: 3, name: article.title, item: `${siteConfig.url}/blog/${article.slug}` }] }]} />
    <header className="article-hero"><div className="container"><Breadcrumbs items={[{ label: "Home", href: "/" }, { label: "Guides", href: "/blog" }, { label: article.title }]} /><span className="eyebrow">The Careya journal · {article.category}</span><h1>{article.title}</h1><p>{article.excerpt}</p><div className="article-meta"><Link href="/editorial-policy">Assisted Living India Editorial Team</Link><span>{article.readTime}</span><span>Updated {new Date(article.updatedAt ?? article.publishedAt).toLocaleDateString("en-IN", { day: "numeric", month: "long", year: "numeric", timeZone: "Asia/Kolkata" })}</span></div></div></header>
    ...
    <article className="prose reading-body"><div className="reading-summary"><span className="eyebrow">At a glance</span><p>{article.summary ?? article.sections[0]?.paragraphs[0] ?? article.excerpt}</p></div>{article.sections.map((section, index) => <section id={sectionId(index)} key={section.heading}><h2>{section.heading}</h2>{section.paragraphs.map((paragraph) => <p key={paragraph}>{paragraph}</p>)}{section.bullets && <ul>{section.bullets.map((bullet) => <li key={bullet}>{bullet}</li>)}</ul>}</section>)}
