...
export default async function ComparePage({ searchParams }: { searchParams: Promise<{ ids?: string }> }) {
  ...
  return <><header className="page-hero"><div className="container"><Breadcrumbs items={[{ label: "Home", href: "/" }, { label: "Compare" }]} /><span className="eyebrow">Side-by-side clarity</span><h1>Compare residences</h1><p>See care, accommodation and facilities together, then request personalised pricing for the options that fit.</p></div></header><section className="section compare-section"><div className="container"><ComparisonView initialIds={ids.split(",").filter(Boolean)} /></div></section></>;
}
