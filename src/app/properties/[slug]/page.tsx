...
import { WriteReviewTrigger } from "@/components/review-form";
...
export default async function PropertyPage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  const sameCityProperties = properties.filter((item) => item.citySlug === property.citySlug && item.id !== property.id);
  const sameStateProperties = properties.filter((item) => item.state === property.state && item.citySlug !== property.citySlug && item.id !== property.id);
  let relatedProperties = sameCityProperties;
  let relatedHeading = `Other residences in ${cityName}`;
  if (relatedProperties.length === 0) {
    relatedProperties = sameStateProperties;
    relatedHeading = `Other residences in ${property.state}`;
  }
  if (relatedProperties.length === 0) {
    relatedProperties = properties.filter((item) => item.featured && item.id !== property.id);
    relatedHeading = "Popular residences to compare";
  }
    ...
    <>
      ...
      <div className="container property-layout">
        <div className="property-main">
          ...
          <section><span className="eyebrow">Profile transparency</span><h2>Source and verification</h2><p>This source-labelled record was last updated on {property.lastUpdated}. {property.claimed ? "The profile is claimed by the operator." : "It is unclaimed and has not been confirmed by the operator."} Google review text is not stored in this profile; numeric ratings are shown only when a source or API snapshot is available.</p><div className="source-link-row"><a className="text-link" href={property.source.url} rel={property.source.url.startsWith("http") ? "nofollow noopener noreferrer" : undefined} target={property.source.url.startsWith("http") ? "_blank" : undefined}>{property.source.label} <ExternalLink size={14} /></a>{property.officialWebsiteUrl && property.officialWebsiteUrl !== property.source.url && <a className="text-link" href={property.officialWebsiteUrl} rel="nofollow noopener noreferrer" target="_blank">Official website <ExternalLink size={14} /></a>}</div></section>
        </div>
        <aside className="property-sidebar">
          <div className="side-card"><PricingTrigger propertyId={property.id} propertyName={property.name} city={city?.name} className="side-card-price pricing-summary-trigger"><small>Personalised monthly fees</small><strong>{pricingLabel}</strong></PricingTrigger><p>Share a few details to receive relevant pricing for your care needs and preferred timeline.</p><PricingTrigger propertyId={property.id} propertyName={property.name} city={city?.name} className="button button-gold">Get pricing</PricingTrigger><WriteReviewTrigger propertyId={property.id} propertyName={property.name} className="button button-ghost" /><Link className="button button-ghost" href={`/concierge?property=${property.slug}`}>Ask our concierge</Link><div className="side-card-trust"><span><Clock3 size={14} /> Typical response target: one business day</span><span><ShieldCheck size={14} /> Your details are only used for this enquiry</span><span><MessageSquareText size={14} /> No booking fee from our directory</span></div></div>
        </aside>
      </div>
      {relatedProperties.length > 0 && <section className="section section-soft">
        <div className="container">
          <SectionHeading eyebrow="Compare nearby" title={relatedHeading} action={{ label: `Open filtered directory`, href: `/browse?city=${property.citySlug}` }} />
          <div className="property-grid">{relatedProperties.map((item) => <PropertyCard property={item} key={item.id} />)}</div>
        </div>
      </section>}
